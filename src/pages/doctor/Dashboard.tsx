...
const DoctorDashboard = () => {
  ...
  const [reportRange] = useState('today');
      ...
      autoTable(doc, {
        ...
        didDrawCell: (data: any) => {
          if (data.section === 'body' && data.column.index === 4) {
            ...
            else if (status === 'Completed') data.cell.styles.textColor = [34, 197, 94];
  ...
  if (!dashData) {
    return (
      <div className="min-h-screen bg-linear-to-br from-gray-50 to-blue-50 flex items-center justify-center md:ml-70">
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-blue-50 md:ml-70 w-full ml-10">
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900">Doctor Dashboard</h1>
              <p className="text-gray-600 mt-2">Welcome back! Here's your practice overview</p>
            </div>
            <button
              onClick={() => setShowReport(true)}
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-linear-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-200 shadow-md text-sm sm:text-base"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 mb-10">
          <div className="bg-linear-to-br from-blue-500 to-blue-600 rounded-2xl p-5 sm:p-6 text-white shadow-lg transform hover:scale-[1.02] transition-transform duration-300">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-blue-100 text-sm mb-1">Total Earnings</p>
                <p className="text-2xl sm:text-3xl font-bold">{formatCurrency(dashData.earnings)}</p>
              </div>
          ...
          <div className="bg-linear-to-br from-emerald-500 to-teal-600 rounded-2xl p-5 sm:p-6 text-white shadow-lg transform hover:scale-[1.02] transition-transform duration-300">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-emerald-100 text-sm mb-1">Appointments</p>
                <p className="text-2xl sm:text-3xl font-bold">{dashData.appointments}</p>
              </div>
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white/20 rounded-full flex items-center justify-center">
                <span className="text-2xl">📅</span>
              </div>
            ...
            <div className="mt-4 pt-4 border-t border-white/20">
              <p className="text-xs sm:text-sm text-emerald-100">
                {dashData.latestAppointments.filter((a: Appointment) => !a.cancelled).length} upcoming
              </p>
            </div>
          </div>

          <div className="bg-linear-to-br from-purple-500 to-pink-600 rounded-2xl p-5 sm:p-6 text-white shadow-lg transform hover:scale-[1.02] transition-transform duration-300">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-purple-100 text-sm mb-1">Total Patients</p>
                <p className="text-2xl sm:text-3xl font-bold">{dashData.patients}</p>
              </div>
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white/20 rounded-full flex items-center justify-center">
                <span className="text-2xl">👥</span>
              </div>
            ...
            <div className="mt-4 pt-4 border-t border-white/20">
              <p className="text-xs sm:text-sm text-purple-100">Active patients this month</p>
            </div>
          </div>
        </div>
                        ...
                        {!item.cancelled && !item.isCompleted && (
                          <div className="flex flex-col sm:flex-row gap-2">
                            <button
                              ...
                              className="px-4 py-2 bg-linear-to-r from-emerald-500 to-teal-600 text-white rounded-lg hover:shadow-md transition-shadow text-sm whitespace-nowrap"
                            >
                              Mark Complete
                            </button>
                            <button
                              ...
                              className="px-4 py-2 border border-red-300 text-red-600 rounded-lg hover:bg-red-50 transition-colors text-sm whitespace-nowrap"
                            >
                              Cancel
                            </button>
                          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
              {/* Modal content remains mostly same - omitted for brevity */}
              {/* ... paste your existing modal content here if needed ... */}
              
              <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t">
                <button
                  onClick={() => setShowReport(false)}
                  className="px-6 py-3 border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 transition-colors font-medium order-2 sm:order-1"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-6 py-3 bg-linear-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:shadow-lg transition-all font-medium flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed order-1 sm:order-2"
                >
                  {isGeneratingPDF ? (
                    <>
